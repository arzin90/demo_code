            ...
            <div class="col-md-8 m-auto">
                <div class="card card-primary">
                    <div class="card-header">
                        <h3 class="card-title">{!! sprintf('Редактировать новости <b>%s</b>', $news->title) !!}</h3>
                    ...
                    <form action="{{route('web-news.news.update',['news'=>$news->id])}}" method="post"
                        ...
                        <div class="card-body">
                            ...
                            <div class="form-group">
                                ...
                                <textarea id="short_description"
                                          ...
                                          rows="4">{{old('short_description')?:$news->short_description}}</textarea>
                            ...
                            <div class="form-group">
                                ...
                                <textarea id="description"
                                          ...
                                          name="description"
                                          rows="4">{{old('description')?:$news->description}}</textarea>
                                @error('description')
                                <span class="error invalid-feedback">{{$message}}</span>
                                @enderror
                            </div>
                            <div class="form-group">
                                ...
                                <select id="category" name="category"
                                    ...
                                    @foreach($categories as $category)
                                        <option
                                            value="{{$category->id}}" {{old('category') && old('category') == $category->id || $category->id == $news->categories[0]->id ?'selected':''}}>{{$category->name}}</option>
                                    @endforeach
                                </select>
                                @error('category')
                                <span class="error invalid-feedback">{{$message}}</span>
                            ...
                            <div class="form-group">
                                ...
                                <select id="location_id" name="location_id"
                                    ...
                                    @foreach($locations as $location)
                                        <option
                                            value="{{$location->id}}" {{old('location_id') && old('location_id')==$location->city || $location->id == $news->location_id ?'selected':''}}>{{$location->city}}</option>
                                    @endforeach
                                </select>
                                @error('location_id')
                                <span class="error invalid-feedback">{{$message}}</span>
                            ...
                            <div class="form-group">
                                ...
                                <select id="status_id" name="status_id"
                                    ...
                                    @foreach($statusList as $key => $status)
                                        <option
                                            value="{{$key}}" {{old('status') && old('status')==$key || $key == $news->status_id ?'selected':''}}>{{$status}}</option>
                                    @endforeach
                                </select>
                                @error('status_id')
                                <span class="error invalid-feedback">{{$message}}</span>
