            ...
            <div class="col-md-8 m-auto">
                <div class="card card-primary">
                    <div class="card-header">
                        <h3 class="card-title">Добавить Формат программы</h3>
                    ...
                    <form action="{{route('web-program-chapter.chapter.store')}}" method="post">
                        @csrf
                        <div class="card-body">
                            ...
                            <div class="form-group">
                                ...
                                <select id="type" name="type"
                                    ...
                                    @foreach($typeList as $key => $type)
                                        <option
                                            value="{{$key}}" {{old('type') && old('type')==$key?'selected':''}}>{{$type}}</option>
                                    @endforeach
                                </select>
                                @error('type')
                                <span class="error invalid-feedback">{{$message}}</span>
